...
# DASHBOARD
    ...
    if dfs:
        ...
        # ========== NOVO: BOTÃO EXPORTAR CONSOLIDADO ==========
        st.divider()
        st.subheader("📥 Exportar Consolidado Completo")

        # Preparar dados para consolidado 22 colunas
        df_consolidado = df.copy()
        
        # Renomear e reordenar colunas conforme esperado
        df_consolidado = df_consolidado.rename(columns={
            "receita": "valor_bruto_item"
        })

        # Adicionar colunas faltantes com valores padrão
        colunas_esperadas = [
            "pack_id", "order_id", "sale_date", "item_id", "seller_sku", 
            "quantity", "unit_price", "valor_bruto_item", "discount_real", 
            "shipment_id", "frete_pago_vendedor"
        ]

        # Manter apenas colunas que existem
        df_consolidado = df_consolidado[[col for col in colunas_esperadas if col in df_consolidado.columns]]
        ...
        data_range = f"{data_inicial.strftime('%Y-%m-%d')}_a_{data_final.strftime('%Y-%m-%d')}"

        st.download_button(
            label="📥 Baixar Consolidado (XLSX)",
            data=output.getvalue(),
            file_name=f"consolidado_{data_range}.xlsx",
