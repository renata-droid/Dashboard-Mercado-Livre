...
if not st.session_state.logado:
    ...
    if st.session_state.consolidado_data is not None:
        ...
        arquivo = os.path.join(
            ...
            "data",
            "consolidado",
            f"consolidado_{data_str}.xlsx"
        )

        if os.path.exists(arquivo):
            with open(arquivo, 'rb') as f:
                st.download_button(
                    label=f"📥 Baixar Consolidado {data_str}",
                    data=f.read(),
            ...
            st.warning(f"⚠️ Arquivo consolidado_{data_str}.xlsx não encontrado. Execute o processamento primeiro.")
