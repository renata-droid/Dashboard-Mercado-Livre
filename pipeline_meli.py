...
def aplicar_regra_frete(df):
    """Aplica a lógica complexa de frete por tracking_number"""
    
    df["frete_regra_calculada"] = df["frete_regra_calculada"]
    ...
    for tracking, grupo in df.groupby("tracking_number"):
        ...
        frete_total = grupo["frete_regra_calculada"].max()
...
def pipeline(data):
    ...
    # Calcular rateio de ads - CORRETAMENTE
    df["quantity"] = df["quantity"].fillna(0).astype(float)
    df["ads_total_item"] = df["ads_total_item"].fillna(0).astype(float)
    
    # Calcular total de quantidade por item (para dividir o custo corretamente)
    total_qtd_por_item = df.groupby("item_id")["quantity"].transform("sum")
    
    # ads_unitario = custo total do item / quantidade total do item no dia
    df["ads_unitario"] = np.where(
        total_qtd_por_item > 0,
        df["ads_total_item"] / total_qtd_por_item,
        0
    )
    
    # ads_rateado = ads_unitario * quantidade desta linha
